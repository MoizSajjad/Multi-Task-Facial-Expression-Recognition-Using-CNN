...
def masked_mse(pred, target):
    # mask invalid target values (-2 → ignore) elementwise
    mask = (target != -2).float()
    if mask.sum() == 0:
...
def main():
    ...
    model = MultiTaskNet(args.backbone, NUM_CLASSES, pretrained=args.pretrained).to(device)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if metrics["cls_acc"] > best_acc:
            ...
            best_path = out_ckpt / f"{args.backbone}_best.pt"
            torch.save({"model": model.state_dict(), "epoch": epoch, "metrics": metrics,
                        "backbone": args.backbone}, best_path)

        # persist logs each epoch
        with open(out_logs / f"{args.backbone}_history.json", "w", encoding="utf-8") as f:
            json.dump(history, f, indent=2)

    elapsed = time.time() - t0
    print(f"Best acc: {best_acc:.4f} | Backbone: {args.backbone} | Time: {elapsed/60:.1f} min | Checkpoint: {best_path}")
